...
class Qubit(ThreeDScene):
    def construct(self):
        ...
        ket_0 = MathTex(r"$\ket{0}$").next_to(axis, np.array([0,0,1]))
        ket_1 = MathTex(r"$\ket{1}$").next_to(axis, -np.array([0,0,1]))
        ket_p = MathTex(r"$\ket{+}$").next_to(axis, RIGHT)
        ket_m = MathTex(r"$\ket{-}$").next_to(axis, LEFT)
        ket_pi = MathTex(r"$\ket{+i}$").next_to(axis, UP)
        ket_mi = MathTex(r"$\ket{-i}$").next_to(axis, DOWN)
        axis_name_group = VGroup(ket_0, ket_1, ket_p, ket_m, ket_pi, ket_mi)
        
        qubit = MathTex(r"$\alpha \ket{0} + \beta \ket{1}$")
        
        gate_list = [MathTex(r'H').to_corner(UP),
                      MathTex(r'$R_z(\frac{\pi}{2})$').to_corner(UP),
                      MathTex(r'$R_x(\frac{\pi}{2})$').to_corner(UP),
                     MathTex(r'$R_y(\frac{\pi}{2})$').to_corner(UP)]
